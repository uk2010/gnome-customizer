...
class CustomizerWindow(Adw.ApplicationWindow):
    ...
    def _migrate_native_theme_ownership(self):
        """Remove dark Yaru names left pinned by legacy releases when GNOME is light."""
        marker="native_theme_ownership_v1"
        if self.state.data.get(marker):return
        schema="org.gnome.desktop.interface"
        if self.settings.get(schema,"color-scheme")=="prefer-light":
            roots={"gtk-theme":Path("/usr/share/themes"),"icon-theme":Path("/usr/share/icons")}
            for key,root in roots.items():
                current=self.settings.get(schema,key)
                if current.startswith("Yaru") and current.endswith("-dark") and (root/current[:-5]).is_dir():self.settings.set(schema,key,current[:-5])
        self.state.data[marker]=True;self.state.save()
    ...
    def _stage_theme(self,directory):
        try:
            ...
            shell=manifest.get("shell",{});schema="io.github.gnomecustomizer.shell";surface_maps={"panel":{"color":"panel-color","color1":"panel-color","color2":"panel-color2","opacity":"panel-opacity","blur":"panel-blur","text_color":"panel-text-color","corner_radius":"panel-radius"},"dock":{"color":"dock-color","color1":"dock-color","color2":"dock-color2","opacity":"dock-opacity","blur":"dock-blur","corner_radius":"dock-radius","icon_size":"dock-icon-size","spacing":"dock-spacing","indicator_style":"dock-indicator-style"},"menus":{"color":"menu-color","color1":"menu-color","color2":"menu-color2","opacity":"menu-opacity","blur":"menu-blur","corner_radius":"menu-radius","text_color":"menu-text-color","border_color":"menu-border-color"},"overview":{"color":"overview-color","opacity":"overview-opacity","blur":"overview-blur","brightness":"overview-brightness","saturation":"overview-saturation"}}
            if shell:self._stage_extension(True,"gnome-customizer@io.github.gnomecustomizer")
            for surface,fields in surface_maps.items():
                if shell.get(surface):
                    enabled_key={"panel":"panel-enabled","dock":"dock-enabled","menus":"menu-enabled","overview":"overview-enabled"}[surface]
                    if self.settings.supports(schema,enabled_key):self.changes.stage(Change("shell",schema,enabled_key,True,f"Enable theme {surface}"))
                ...
                if surface in {"panel","dock","menus"} and shell.get(surface):
            ...
            self._update_login_preview();self._pending();self.toast(f"{manifest['name']} staged")
    ...
    def _extend_dock(self,page):
        g=Adw.PreferencesGroup(title="Shell Companion",description="Enabling preserves every unrelated extension");page.add(g);row=Adw.SwitchRow(title="Enable GNOME Customizer Companion");enabled=self.settings.get("org.gnome.shell","enabled-extensions");uuid="gnome-customizer@io.github.gnomecustomizer";row.set_active(uuid in enabled)
        row.connect("notify::active",lambda r,_:self._stage_extension(r.get_active(),uuid));g.add(row)
