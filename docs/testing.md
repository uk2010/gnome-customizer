...
- The Ubuntu Factory Defaults action resets every exposed desktop, Shell, dock, application, wallpaper, display, and GDM setting regardless of which customization tool changed it, while preserving settings outside the app's scope.
...
- The full application constructs with all detected GNOME 50 schemas under an isolated display (14 Desktop pages, 7 Login Screen pages, and zero startup-staged changes). Wide and collapsed navigation layouts were exercised without GTK/Libadwaita layout warnings.
