...
To snapshot the applied appearance, open **Themes** and choose **Save as Theme**. The snapshot uses the same mapping when restored and includes every supported control on Appearance, Fonts/Icons/Cursor, Dock, Blur, and Top Bar, plus the selected sound theme. Hardware and behavioral pages such as input, power, Night Light, and displays are intentionally not appearance themes. A newer `minimum_gnome` produces a blocking compatibility message; a newer runtime beyond `maximum_tested_gnome` produces a warning.
