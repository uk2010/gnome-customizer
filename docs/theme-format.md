...
To snapshot the applied appearance, open **Themes** and choose **Save as Theme**. To author one manually, use the in-app Theme Builder, preview each surface, enter metadata, and choose Export Theme. Imported version-1 themes can be opened for editing after validation. A newer `minimum_gnome` produces a blocking compatibility message; a newer runtime beyond `maximum_tested_gnome` produces a warning.
