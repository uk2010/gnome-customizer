...
class ShellExtensionTests(unittest.TestCase):
    ...
    def test_overview_uses_blurred_wallpaper_backgrounds(self):
        ...
        self.assertIn("tint:null", extension)
        self.assertIn("if (opacity <= 0)", extension)
        self.assertIn("item.tint?.destroy()", extension)
        self.assertIn("item.tint=null", extension)
        self.assertIn("colorWithOpacity(color, opacity)", extension)
        self.assertIn("if (!item.tint)", extension)
        self.assertIn("effectiveBrightness=opacity<=0 ? 1 : brightness",extension)
        self.assertIn("effectiveSaturation=opacity<=0 ? 1 : saturation",extension)
        self.assertIn("manager.backgroundActor.content.brightness=1",extension)
        self.assertIn("manager.connect('changed',neutralizeWallpaper)",extension)
        self.assertIn("neutralWallpapers",extension)
        self.assertIn("desaturateActors",extension)
    ...
    def test_app_folder_tiles_and_dialogs_have_independent_transparency(self):
        ...
        self.assertIn("if (hovered) {",extension)
        self.assertIn("this._settings.get_double('overview-hover-opacity')",extension)
        self.assertIn("this._settings.get_string('overview-hover-color')",extension)
        self.assertIn("Uses the overview hover tint and opacity when a folder is highlighted",preferences)
        self.assertIn("actor.set_style(record.style)",extension)
        self.assertIn("background-color: ${color}",extension)
